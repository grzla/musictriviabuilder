...
const transformData = (parsedData) => {
  return parsedData.map(entry => {
    ...
    return {
      ...
      title: entry['Title'],
      year: null,
      releaseYear: entry['Release Year'] ? parseInt(entry['Release Year'], 10) : null,
    };
  });
};
...
const insertDataToDB = (data) => {
  const query = 'INSERT INTO usedsongs (Artist, Title, ReleaseYear) VALUES ?';
  const values = data.map(entry => [entry.artist, entry.title, entry.releaseYear]);
...
// Example usage
processTSVFile('/home/greg/Documents/wrikMusicTrivia/usedSongs.tsv');